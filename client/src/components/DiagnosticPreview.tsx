...
function DiagnosticCard({ item, index, reduced }: DiagnosticCardProps) {
  return (
    ...
    >
      ...
      {/* Passo badge — retangular, cantos suaves */}
      <span className="relative inline-flex items-center gap-2 rounded-md border border-primary/45 bg-primary/[0.06] px-2.5 py-1 text-[10.5px] font-semibold uppercase tracking-[0.20em] text-primary">
        <span aria-hidden className="h-1 w-1 rounded-full bg-primary" />
        Passo {item.index}
      </span>

      {/* Title em primary + body */}
      <h3 className="relative mt-7 text-xl font-bold tracking-[-0.015em] text-primary">
        {item.title}
      </h3>
